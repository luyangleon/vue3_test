<template>
    ...
    <!-- toRef和toRefs -->
    <h2>toRef的name:{{name2}}</h2>
    <h2>toRef的age:{{age2}}</h2>
    <h2>toRef的salary:{{salary2}}K</h2>
    <h2>toRefs的salary:{{job2.j1.salary}}K</h2>
  </template>
  
  <script>
  import { reactive, ref, computed, watch, toRef, toRefs } from 'vue'
  ...
  export default {
    ...
    setup(props, context) {
      ...
      return {
        ...
        lpList,
        watchObj,
        name2:toRef(toRefObj,'name2'),
        age2:toRef(toRefObj,'age2'),
        salary2:toRef(toRefObj.job2.j1,'salary'),
        ...toRefs(toRefObj),
        sayHi,
        changeLP,
        invokeHello
